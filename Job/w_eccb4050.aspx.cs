...
public partial class Job_w_eccb4050 : System.Web.UI.Page
{
    ...
    public static string Update(cSaveData DATA)
    {
        ...
        try
        {
            ...
            //---------------------------------------------------------------------------
            if (DATA.getFirst().getQuery() == "w_eccb4050_M_1"
                && DATA.getFirst().getFirst().getType() == typeQuery.INSERT)
            {
                ...
                try
                {
                    ...
                    objProcedure.objCmd.Parameters.AddWithValue("@KeyType", "ECA");
                ...
                for (int iAry = 0; iAry < DATA.getSize(); iAry++)
                {
                    ...
                    {
                        case "w_eccb4050_S_1":
                        case "w_eccb4050_S_2_1":
                        case "w_eccb4050_S_2_2":
                        case "w_eccb4050_S_6":
                            DATA.getObject(iAry).setValues("root_no", strKey);
                            DATA.getObject(iAry).setValues("root_seq", "1");
                            break;
                        default:
                            DATA.getObject(iAry).setValues("eca_no", strKey);
                            break;
                    }
                }
            }
            ...
            for (int iAry = 0; iAry < DATA.getSize(); iAry++)
            {
                ...
                for (int iRow = 0; iRow < DATA.getObject(iAry).getSize(); iRow++)
                {
                    if (DATA.getObject(iAry).getRow(iRow).getType() == typeQuery.INSERT)
                    {
                        ...
                        if (strID.Equals("EC_MODEL"))
                            model_seq = iKey;
                        else if (strID.Equals("EC_MODEL_D"))
                        {
                            if (DATA.getObject(iAry).getRow(iRow).getValue("model_seq") == "" || DATA.getObject(iAry).getRow(iRow).getValue("model_seq") == "0")
                                DATA.getObject(iAry).setValue(iRow, "model_seq", model_seq.ToString());
                        }
