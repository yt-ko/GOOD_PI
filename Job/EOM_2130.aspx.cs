...
public partial class JOB_EOM_2130 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
    }

    private static string crtNewKey(string _KeyId)
    {
        if (_KeyId != "ECCB") return "";
        ...
        try
        {
            objProcedure.objCmd.CommandText = "SP_KEYGEN_ECCB";
        ...
        catch (SqlException ex)
        {
            ...
            string sMsg = "Key를 생성할 수 없습니다.\n- " + ex.Message;
            throw new Exception( new JavaScriptSerializer().Serialize( new entityProcessed<string>( codeProcessed.ERR_SQL, sMsg) ) );
        }
        catch (Exception ex)
        {
            objProcedure.processTran(doTransaction.ROLLBACK);
            string sMsg = "Key 생성 중에 오류가 발생하였습니다.\n- " + ex.Message;
            throw new Exception(new JavaScriptSerializer().Serialize(new entityProcessed<string>(codeProcessed.ERR_PROCESS, sMsg)));
        }
        sNewKey = objProcedure.objCmd.Parameters["@NewKey"].Value.ToString(); 
    ...
    public static string Update(cSaveData DATA)
    {
        ...
        try
        {
            ...
            {
                string strKey = crtNewKey("EOM_PLAN");
