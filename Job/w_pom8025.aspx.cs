...
public partial class Job_w_pom8025 : System.Web.UI.Page
{
    ...
    public static string Mail(cProcedureData DATA)
    {
        ...
        try
        {
            ...
            StreamWriter objIO = new StreamWriter(strPath + "20120201" + ".html", false, System.Text.Encoding.UTF8);
            ...
            try
            {
                ...
                for (int iAry = 0; iAry < iRows; iAry++)
                {
                    ...
                    strQuery = string.Format(@"
                        SELECT
	                         ISNULL(R.supp_nm, '&nbsp;') AS supp_nm
	                        ,ISNULL(R.part_nm, '&nbsp;') AS part_nm
	                        ,ISNULL(SUBSTRING(R.need_date, 5, 2) + '/' + SUBSTRING(R.need_date, 7, 2), '&nbsp;') AS req_date
	                        ,ISNULL(SUBSTRING(R.plan_date_to, 5, 2) + '/' + SUBSTRING(R.plan_date_to, 7, 2), '&nbsp;') AS plan_date
	                        ,ISNULL(CAST(DATEDIFF(DAY, R.plan_date_to, R.need_date) AS VARCHAR), '&nbsp;') AS date_diff
                            ,CASE WHEN R.delay_yn = '1' THEN '+' ELSE '-' END AS delay_yn
	                        ,CASE WHEN R.delay_yn = '1' THEN '#A00000' ELSE '#000000' END AS color
                        FROM [fn_getProjectBom] ('{0}', '%', '0') R
                        ",
                        strOrders[iAry]
                    );
                    objDr = (new cDBQuery(
                    ...
                    while (objDr.Read())
                    {
                        ...
                        strBody =
                            ...
                            + "<font face=굴림체 color=" + objDr["color"] + ">" + objDr["req_date"] + "</font>"
                            + "</td>"
                            + "<td colspan=5 height=24 align=center valign=middle>"
                            + "<font face=굴림체 color=" + objDr["color"] + ">" + objDr["plan_date"] + "</font>"
                            + "</td>"
                            + "<td colspan=5 height=24 align=center valign=middle>"
                            + "<font face=굴림체 color=" + objDr["color"] + ">"
                            + ((objDr["date_diff"].ToString() == "0" || objDr["date_diff"].ToString() == "&nbsp;") ? "" : objDr["delay_yn"]) 
                            + objDr["date_diff"]
                            + "</font>"
                            + "</td>"
			                + "</tr>";
