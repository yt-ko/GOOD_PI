...
public partial class Job_TDR_1010 : System.Web.UI.Page
{
    ...
    public static string Print(cRetrieveData DATA)
    {
        ...
        try
        {
            ...
            // Set Path & File Name
            string strRoot = HttpContext.Current.Server.MapPath("~/") + @"Report\";
            string sFormPath = strRoot + @"Forms\";
            string sFormFile = (strType == "rpt1") ? "TDR_표준기술자료제공동의서" : "TDR_기술자료제공요청서";
            string sFormExt = ".docx";
            string strSource = sFormPath + sFormFile + sFormExt;

            string sOutPath = strRoot + strPage + @"\";
            string sOutFile = sKeyNo + "_" + sUserId;   // Return File Name
            if (sOutExt.IndexOf(".") != 0) sOutExt = "." + sOutExt;
            string strTarget = sOutPath + sOutFile + sFormExt;

            #endregion

            // Create Target File from Form File
            try
            {
            ...
            catch
            {
                throw new Exception(
                    new JavaScriptSerializer().Serialize(
                        new entityProcessed<string>(codeProcessed.ERR_PROCESS,"파일 생성 중 오류가 발생하였습니다.(1)"))
                    );
            }

            RichEditDocumentServer docServer = new RichEditDocumentServer();
            docServer.LoadDocument(strTarget, DocumentFormat.OpenXml);
            Document wDoc = docServer.Document;
            ...
            try
            {
                ...
                foreach (string f1 in strFileds)
                {
                    ...
                    for (int i = 0; i < wDoc.Fields.Count; i++)
                    {
                        string sCol = wDoc.GetText(wDoc.Fields[i].CodeRange).Split('\\')[0].Replace("\"", "");
                        sCol = sCol.Substring(12).Trim();

                        if (f1.Equals(sCol))
                        {
                            wDoc.Selection = wDoc.Fields[i].Range;
                            //wDoc.Replace(wDoc.Fields[i].CodeRange, val);
                            wDoc.Replace(wDoc.Selection, val);  // 현재 Field 삭제됨
                            i--;    // 중복 Field 있을 경우 필요. 
                        }
                    }
                    // 사용 불가 : Collection 개체 수정 오류 발생
                    //foreach (Field fld in wDoc.Fields)
                    //{
                    //    string sCol = wDoc.GetText(fld.CodeRange).Split('\\')[0].Replace("\"", "");
                    //    sCol = sCol.Substring(12).Trim();
                    //    if (f1.Equals(sCol))
                    //    {
                    //        wDoc.Selection = fld.Range;
                    //        wDoc.Replace(wDoc.Fields[i].CodeRange, val);
                    //        wDoc.Replace(wDoc.Selection, val);
                    //    }
                    //}
                }
                ...
                // Save Document
                docServer.EndUpdate();
                wDoc.SaveDocument(strTarget, DocumentFormat.OpenXml);

                // Export to PDF
                if (sOutExt.ToUpper().Equals(".PDF"))
                    docServer.ExportToPdf(sOutPath + sOutFile + sOutExt);
            }
            catch (Exception ex)
            {
                throw new Exception(
                    new JavaScriptSerializer().Serialize( 
                        new entityProcessed<string>( codeProcessed.ERR_PROCESS, "파일 생성 중 오류가 발생하였습니다.\n- " + ex.Message) )
                    );
            }
            finally
            {
                if (docServer != null) docServer.Dispose();
            }
            #endregion

            strReturn = new JavaScriptSerializer().Serialize(new entityProcessed<string>(codeProcessed.SUCCESS, sOutFile + sOutExt));
    ...
    #region PrintWord() : MS Word 활용
    [WebMethod]
    //public static string PrintWord(cRetrieveData DATA)
    //{
