...
public partial class JOB_SYS_2050_RPT_3010 : System.Web.UI.Page
{
    ...
    public static string Update(cSaveData DATA)
    {
        ...
        try
        {
            ...
            //---------------------------------------------------------------------------
            if (DATA.getFirst().getQuery() == "SYS_2050_S_1"
                && DATA.getFirst().getFirst().getType() == typeQuery.INSERT)
            {
                string strKey = DATA.getFirst().getFirst().getValue("rpt_id");
                DATA.setValues("rpt_id", strKey);
            }
            //---------------------------------------------------------------------------
                            ...
                            try
                            {
                                objUpdate.objDr = (new cDBQuery(
                                                        ...
                                                        "SELECT dbo.FN_CREATEKEY('" + strID + "','" +
                                                            DATA.getValue(iAry, iRow, "rpt_id") + "')"
