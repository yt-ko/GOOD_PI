...
public partial class Job_ECM_2020_jj : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (Path.GetExtension(strSource).Equals(".doc") || Path.GetExtension(strSource).Equals(".docx"))
            {
                ...
                try
                {
                    ...
                    // 서명 : Footer에 인증문구 삽입 & 서명란에 도장 이미지 삽입
                    if (strPstat.Equals("CFM"))
                    {
                        // 인증문구 삽입
                        string footerText = "본 계약서는 전자서명법에 따라 (주)원익아이피에스와 상기 업체 간에 공인인증을 득하여 체결한 전자계약서로써 제반 법령에 따라 법적 효력을 충족한 것임을 확인합니다.";
                        wDoc.setFooterText(footerText, "DarkBlue10");
                        ...
                        foreach (string fieldNm in wDoc.getMergeFiledCode()) // Loop Field Codes of Document 
                        {
                            ...
                            string signFile = strRoot + @"\Files\ECM_FILES\SIGN_FILES\" + getSignImage(strDocID, fieldNm) + ".png";
                            wDoc.setFiledImage(fieldNm, signFile);
                        }
                        wDoc.endUpdate();
                ...
                try
                {
                    wDoc.exporToPdf(strTarget); // by JJJ
