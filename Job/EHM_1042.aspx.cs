...
public partial class JOB_EHM_1042 : System.Web.UI.Page
{
    ...
    public static string Update(cSaveData DATA)
    {
        ...
        try
        {
            ...
            {
                if (DATA.getObject(iAry).getQuery() != "EHM_1040_M_2")
                    continue;
                int iKey = 0;
                for (int iRow = 0; iRow < DATA.getObject(iAry).getSize(); iRow++)
                {
                    if (DATA.getObject(iAry).getRow(iRow).getType() == typeQuery.INSERT)
                    {
                        if (iKey == 0)
                        {
                            try
                            {
                                objUpdate.objDr = (new cDBQuery(
                                                        ruleQuery.INLINE,
                                                        "SELECT ISNULL(MAX(status_seq), 0) + 1" +
                                                        "  FROM AS_PART_S" +
                                                        " WHERE part_tp = '" + HttpUtility.UrlDecode(DATA.getValue(iAry, iRow, "part_tp")) + "'"
                        ...
                        DATA.setValue(iAry, iRow, "status_seq", Convert.ToString(iKey++));
                    }
                }
            }
            //---------------------------------------------------------------------------
