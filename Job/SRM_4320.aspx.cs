    ...
    public static string Update(cSaveData DATA)
    {
        ...
        try
        {
            ...
            for (int iAry = 0; iAry < DATA.getSize(); iAry++)
            {
                ...
                {
                    // 창고 체크
                    if (DATA.getObject(iAry).getQuery() == "SRM_4320_S_1")
                    {
                        string wh_cd = DATA.getValue(iAry, iRow, "wh_cd");
                        string plant_cd = DATA.getValue(iAry, iRow, "plant_cd");
                        try
                        {
                            objUpdate.objDr = (new cDBQuery(
                                                        ruleQuery.INLINE,
                                                        string.Format("SELECT rcode FROM ZCODED WHERE hcode = 'ISCM10' AND dcode = '{0}'", wh_cd)
                                                    )).retrieveQuery(objUpdate.objCon);
                            if (objUpdate.objDr.Read())
                            {
                                if (plant_cd != objUpdate.objDr[0].ToString())
                                {
                                    throw new Exception(
                                            new JavaScriptSerializer().Serialize(
                                                new entityProcessed<string>(
                                                        codeProcessed.ERR_PROCESS,
                                                        "선택한 창고의 공장과 발주서의 공장이 일치하지 않습니다.")
                                            )
                                        );
                                }
                            }
