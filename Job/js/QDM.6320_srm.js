...
var gw_job_process = {
    ...
    ready: function () {
        ...
        if (gw_com_module.v_Session.USER_TP == "SUPP")
            v_global.logic.IsSupp = true;
        else
            v_global.logic.IsSupp = false;
        ...
        var args = { type: "PAGE", page: "TDR_NcrAgree", title: "요청내역 및 자료제공 동의", width: 800, height: 500 };
        gw_com_module.dialoguePrepare(args);
    ...
    UI: function () {
        ...
        var args = {
            ...
            element: [
                { name: "상세", value: "발생정보", icon: "실행" }, //SUPP
                { name: "상신", value: "결재상신", icon: "기타", updatable: true },
                { name: "저장", value: "저장" },    //SUPP
                { name: "요청", value: "발생내용 수정요청", icon: "기타", updatable: true },
                { name: "통보", value: "결과통보", icon: "기타" },  //SUPP
                { name: "재발", value: "초발/재발", icon: "기타", updatable: true },
                { name: "닫기", value: "닫기" } //SUPP
            ]
        };
        gw_com_module.buttonMenu(args);
        if (v_global.logic.IsSupp) {
            gw_com_api.hide("lyrMenu_Main", "상신", false);
            gw_com_api.hide("lyrMenu_Main", "요청", false);
            gw_com_api.hide("lyrMenu_Main", "재발", false);
        }
        //=====================================================================================
        var args = {
        ...
        if (v_global.logic.IsSupp) gw_com_api.hide("lyrMenu_Sub");
        ...
        var args = {
            ...
            element: [
                ...
                { header: "제공동의일시", name: "agree_dt", width: 80, align: "left" },
...
function processSave(param) {
    ...
    // 요청상세 및 기술자료제공 동의
    if (v_global.logic.IsSupp) {
        var IsAgree = gw_com_api.getValue("grdData_Sub", "selected", "agree_yn", true);
        if (IsAgree != "Y") {
