...
var gw_job_process = {
    ...
    UI: function () {
        ...
        if (v_global.logic.supp == "1") {
            ...
            if (v_global.logic.view != "1") {
                ...
                args.element.push({ name: "DeleteA", value: "삭제", icon: "삭제" });
            }
        }
    ...
    procedure: function () {
        ...
        gw_com_module.eventBind(args);
        var args = { targetid: "lyrMenu_Top", element: "DeleteA", event: "click", handler: processBatch };
        gw_com_module.eventBind(args);
...
function processFileUpload(param) {
    ...
    // prepare dialogue.
    var args = {
        type: "PAGE", page: "SYS_FileUpload", title: "Upload Fils", datatype: dataType  //, open: true
...
function processBatch(param) {
    ...
    if (jobCd == "DeleteA") {
        // 7일 경과전 삭제 불가
        var upd_day = gw_com_api.getValue("frmData_SubA", 1, "upd_day");
        if (upd_day < 8) {
            gw_com_api.messageBox([{ text: "등록 후 7일 경과 전에는 삭제 할 수 없습니다." }], 500);
            return;
        }
        // 삭제 진행 확인
        param.element = "Delete";
        gw_com_api.messageBox([{ text: "삭제된 제안서는 복구할 수 없습니다." }, { text: "삭제 처리를 진행하시겠습니까?" }], 420, gw_com_api.v_Message.msg_confirmBatch, "YESNO", param);
        return;
    }
...
function processBatchSuccess(response, param) {
    ...
    // Refresh Data
    if (param.act != "CheckCompNo") {
        v_global.logic.key = "0";
        processRetrieve({ key: "0", del: true });
    }
    else if (param.act != "CheckCompNo")
...
function processRetrieve(param) {
    ...
    if (param.sub == undefined) { // Main Data Box
        var args = {
            ...
            , handler: { complete: processRetrieveComplete, param: { key: param.key, sub: true, del: param.del } } // for sub data box
        };
        gw_com_module.objRetrieve(args);
    }
...
function processRetrieveComplete(param) {
    if (param.del != undefined && param.del == true) {
        processReloadList({});
    }
    else if (v_global.logic.key == "0") {
        processReloadList({});
    }
    else if (param.agree) processMemoEdit(param);
    else if (param.sub) {
        // Sub Data Box
...
streamProcess = function (param) {
    ...
    switch (param.ID) {
        ...
        case gw_com_api.v_Stream.msg_openedDialogue:
            {
                if (param.from.page == "SYS_FileUpload") { processFileUpload(param); return; }
