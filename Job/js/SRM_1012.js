...
var gw_job_process = {
    ...
    UI: function () {
        ...
        var args = {
            ...
            element: [
                ...
                //{ name: "복사", value: "복사추가", icon: "추가", updatable: true },   // 입찰견적관리(SRM_1011)로 이동
                { name: "삭제", value: "삭제" },
                { name: "저장", value: "저장" },
                { name: "닫기", value: "닫기" }
            ]
        ...
        var args = {
            ...
            element: [
                ...
                { name: "추가", value: "품목추가" },
                { name: "btnDwgDist", value: "도면맵핑", icon: "기타" },
                { name: "btnDwgSend", value: "배포시작", icon: "기타" },
                { name: "삭제", value: "삭제" }
            ]
        };
        //----------
        ...
        var args = {
            ...
            element: [
                { name: "업체등록", value: "신규 협력사 등록", icon: "기타", updatable: true },
                { name: "추가", value: "추가" },
                { name: "삭제", value: "삭제" },
        ...
        var args = {
            ...
            element: [
                ...
                { header: "배포번호", name: "dwg_dist_no", width: 72, align: "center", editable: { type: "hidden", width: 70, maxlength: 15 } },
        ...
        var args = {
            ...
            element: [
                ...
                { name: "pstat", editable: { type: "hidden" }, hidden: true },
                { name: "temp_yn", editable: { type: "hidden" }, hidden: true },
    ...
    procedure: function () {
        ...
        gw_com_module.eventBind(args);
        var args = { targetid: "lyrMenu_PER_ITEM", element: "btnDwgDist", event: "click", handler: processDwgDist };
        gw_com_module.eventBind(args);
        var args = { targetid: "lyrMenu_PER_ITEM", element: "btnDwgSend", event: "click", handler: processDwgSend };
        gw_com_module.eventBind(args);
        //----------
        var args = { targetid: "lyrMenu_PER_ITEM", element: "삭제", event: "click", handler: processButton };
        gw_com_module.eventBind(args);
        //=====================================================================================
        var args = { targetid: "lyrMenu_PER_SUPP", element: "업체등록", event: "click", handler: processButton };
        gw_com_module.eventBind(args);
        //----------
                ...
                case "취소":
                    {
                        ...
                        if (cnt > 0) {
                            ...
                            //도면배포 시작 처리
                            var IsDwgSend = false;
                            if (param.element.indexOf("의뢰") > -1) IsDwgSend = true;
                            ...
                            var args = {
                                ...
                                handler: {
                                    success: successSave,
                                    param: { type: "PUB", to: to, temp_id: (stat == "PUB" ? "SRM_PER01" : "SRM_PER02"), supp_seq: supp_seq, stat: stat, IsDwgSend: IsDwgSend }
                                }
                            };
...
function processExport(param) {
    ...
    var rmk1 = "** 참고사항 :  의뢰 접수 후 단가 및 납기 협의, 납품가능일 표기 바람.\n" +
        ...
        "** 납품장소 :  본사";
    var rmk2 = "비고 : " + gw_com_api.getValue("frmData_PER", 1, "per_rmk", false, true);
    ...
    var args = {
        ...
        option: [
            ...
            { name: "PER_NO", value: gw_com_api.getValue("frmData_PER", 1, "per_no", false, true) },
            { name: "ITEM_CNT", value: gw_com_api.getRowCount("grdData_PER_ITEM") },
            { name: "PER_DATE", value: gw_com_api.Mask(gw_com_api.getValue("frmData_PER", 1, "per_date", false, true), "date-ymd") },
            { name: "PER_COMP", value: gw_com_api.getValue("frmData_PER", 1, "per_comp", false, true) },
            { name: "PER_MAN", value: gw_com_api.getValue("frmData_PER", 1, "per_man", false, true) },
            { name: "PER_TEL", value: gw_com_api.getValue("frmData_PER", 1, "per_telno", false, true) },
            { name: "PER_FAX", value: gw_com_api.getValue("frmData_PER", 1, "per_faxno", false, true) },
            { name: "PER_EMAIL", value: gw_com_api.getValue("frmData_PER", 1, "per_email", false, true) },
            ...
            { name: "CLOSE_DATE", value: gw_com_api.getValue("frmData_PER", 1, "per_term", false, true) },
...
function successDwgSend(response) {
    var run = true;
    var rCode = ""; var rDesc = "";
    $.each(response, function () {
        $.each(this.KEY, function () {
            if (this.NAME == "returnCode") rCode = this.VALUE;  // F, 
            else if (this.NAME == "returnDesc") rDesc = this.VALUE;
            return run;
        });
    });
    if (rCode == "S") rDesc = "도면배포를 시작하였습니다.";
    gw_com_api.messageBox([{ text: rDesc }]);
    return run;
}
function errorDwgSend(response) {
    var run = true;
    $.each(response, function () {
        $.each(this.KEY, function () {
            if (this.NAME == "returnDesc") {
...
function cpmpleteDwgSend(response) {
    var run = true;
    $.each(response, function () {
        $.each(this.KEY, function () {
            if (this.NAME == "nt_seq") {
...
//----------
function processDwgSend(param) {

    if (gw_com_api.getRowCount("grdData_PER_ITEM") < 1 || gw_com_api.getValue("grdData_PER_ITEM", 1, "dwg_dist_no", true).length < 2)
        return;
    // Page Method 활용
    var params = {
        DATA: {
            ...
            OPTION: {
                ...
                VALUE: ["45506061143079606660614654"
                    , gw_com_api.getValue("grdData_PER_ITEM", 1, "dwg_dist_no", true)
                    , gw_com_module.v_Session.USR_ID
                    , gw_com_module.v_Session.USR_NM]
            }
        }
    };
    var param = {
        request: "SERVICE",
        url: "SRM_1012.aspx/DwgSend",
        params: JSON.stringify(params),
        handler_success: successDwgSend,
        handler_error: errorDwgSend //,        handler_complete: cpmpleteDwgSend
    };
    gw_com_module.callRequest(param);

    return;
    // JavaScript jQuery 활용 방법
    var settings = {
        "url": "http://dist.ips.co.kr/rest/svc_dist/startExternalDistBidd",
        "method": "POST",
        "timeout": 0,
        "headers": {
            "Content-Type": "application/json"
        },
        "data": JSON.stringify({ "secureId": "45506061143079606660614654", "distMasterNumber": "DO2101112", "userId": "admin", "userName": "admin" }),
    };
    ...
    //gw_com_site.startExternalDiskBidd(args);
...
function processDwgDist(param) {
    ...
    if (gw_com_api.getRowCount("grdData_PER_ITEM") > 0)
        sDistNo = gw_com_api.getValue("grdData_PER_ITEM", 1, "dwg_dist_no", true);

    if (param.element == "grdData_PER_ITEM")
        sDistNo = gw_com_api.getValue("grdData_PER_ITEM", param.row, param.element, true);

    v_global.logic.search = { dist_no: sDistNo, mode: sMode };
        ...
        case gw_com_api.v_Stream.msg_closeDialogue:
            {
                switch (param.from.page) {
                    ...
                    case "SRM_BID_DwgDist_View":
                        if (param.data != undefined && param.data.edit != undefined) {
                            if (param.data.edit == "save") {
                                var ids = gw_com_api.getRowIDs("grdData_PER_ITEM");
                                $.each(ids, function () {
                                    gw_com_api.selectRow("grdData_PER_ITEM", this, true);
                                    gw_com_api.setValue("grdData_PER_ITEM", this, "dwg_dist_no", param.data.dwg_dist_no, true, true, true);
                                    if (gw_com_api.getCRUD("grdData_PER_ITEM", this, true) == "none" || gw_com_api.getCRUD("grdData_PER_ITEM", this, true) == "retrieve")
                                        gw_com_api.setCRUD("grdData_PER_ITEM", this, "modify", true);
                                });
                            }
                            else if (param.data.edit == "delete") {
                                var ids = gw_com_api.getRowIDs("grdData_PER_ITEM");
                                $.each(ids, function () {
                                    gw_com_api.selectRow("grdData_PER_ITEM", this, true);
                                    gw_com_api.setValue("grdData_PER_ITEM", this, "dwg_dist_no", " ", true, true, true);
                                    if (gw_com_api.getCRUD("grdData_PER_ITEM", this, true) == "none" || gw_com_api.getCRUD("grdData_PER_ITEM", this, true) == "retrieve")
                                        gw_com_api.setCRUD("grdData_PER_ITEM", this, "modify", true);
                                });
                            }
                            ...
                            //        var row = gw_com_api.getFindRow("grdData_PER_ITEM", "item_no", item_no)
                            //        if (row > 0) {
                            //            gw_com_api.setValue("grdData_PER_ITEM", row, "dwg_dist_no", this.dwg_dist_no);
                            //        }
                            //    });
                            //}
                        }
                    ...
                    case "SYS_2019":
                        {
                            if (param.data != undefined) {
                                var row = [{
                                    ...
                                    supp_man: param.data.emp_nm + (param.data.emp_pos_nm == undefined || param.data.emp_pos_nm == "" ? "" : " " + param.data.emp_pos_nm),
