...
function processLink(param) {
   ...
   /* else return;*/
    ...
    gw_com_module.objRetrieve(args);
    processFileList({
        data_key: gw_com_api.getValue("frmData_Main", "selected", "rqst_no"), data_seq:-1
    });
...
function processUpload(param) {
    ...
    // Set File Data Type
    var dataType = "NCR-RQST";
    var dataKey = gw_com_api.getValue("frmData_Main", 1, "rqst_no");   // Main Key value for Search

    // Set Argument Data for Popup Page Interface : 첫번째 Open 시의 메시지 전달을 위함
    v_global.event.data = { data_tp: dataType, data_key: dataKey, data_seq: 0, user: gw_com_module.v_Session.USR_ID, crud: "C", rev: 0, revise: false }; // additional data = { data_subkey: "", data_subseq:-1 }
    ...
    var args = {
        ...
        width: 660, height: 260, scroll: true,  // multi( h:350, scroll) / single(h:300)
...
streamProcess = function (param) {
    ...
    switch (param.ID) {
        ...
        case gw_com_api.v_Stream.msg_closeDialogue: {
            switch (param.from.page) {                   // 2021-05-13
                case "SYS_FileUpload": {
                    
                    processFileList({
                        data_key: gw_com_api.getValue("frmData_Main", "selected", "rqst_no")
                    });
