...
var gw_job_process = {
    ...
    ready: function () {
        ...
        // Get Page Parameters
        v_global.logic.view = gw_com_api.getPageParameter("view");
    ...
    UI: function () {
        ...
        var args = {
            tabid: "lyrFrame",
            target: [
                { type: "PAGE", id: "SRM_OpenSrc_List", title: "제안 현황", launch: true, handler: clickTabPage },   //tabselect Event handler
                { type: "PAGE", id: "SRM_OpenSrc_Edit", title: "제안 상세" }
            ]
            //, data: { param: [{ name: "nt_tp", value: "협력사" }] }
        };
        if (v_global.logic.view != "1") {
            args.target.push({ type: "PAGE", id: "SRM_OpenSrc_RptA", title: "월별 집계" });
        }
        //gw_com_module.v_Current.menu_args = "&mode=0";   // add Menu Arguments
        ...
        case gw_com_api.v_Stream.msg_refreshPage: // List page 를 Refresh 하기위한 처리
            {
                ...
                $("#lyrFrame").tabs('select', "SRM_OpenSrc_List");
            } break;
