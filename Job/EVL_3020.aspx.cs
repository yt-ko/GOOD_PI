...
public partial class JOB_EVL_3020 : System.Web.UI.Page
{
            ...
            try
            {
                ...
                try
                {
                    ...
                    using (ZipFile z = new ZipFile())
                    {
                        ...
                        //z.ProvisionalAlternateEncoding = System.Text.Encoding.GetEncoding(866);
                        //z.UseUnicodeAsNecessary = true;
                        z.AlternateEncodingUsage = ZipOption.AsNecessary;
                        while (objDr.Read())
                        {
                            string strSrc = string.Format("{0}{1}.{2}", objDr["file_path"].ToString(), objDr["file_id"].ToString(), objDr["file_ext"].ToString());
                            string strName = objDr["file_nm"].ToString();
                            if (File.Exists(strSrc))
                            {
                                int i = 0;
                                string a = Path.GetFileNameWithoutExtension(strName);
                                string e = Path.GetExtension(strName);
                                while (z.ContainsEntry(strName))
                                {
                                    i++;
                                    strName = string.Format("{0} ({1}).{2}", a, i, e);
                                }
                                z.AddFile(strSrc).FileName = strName;
                                bAdd = true;
                            }
                        }
                        objDr.Close();
