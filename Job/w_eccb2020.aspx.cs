        ...
        try
        {
                ...
                && DATA.getFirst().getFirst().getType() == typeQuery.INSERT)
            {
                ...
                try
                {
                    ...
                    string keyType = DATA.OBJECTS[0].getValue(0, "eccb_tp");
                    if (string.IsNullOrEmpty(keyType))
                        keyType = "ECCB";

                    objProcedure.objCmd.CommandText = strSQL;
                    objProcedure.objCmd.Parameters.AddWithValue("@KeyType", keyType);
            ...
            try
            {
                string strQuery = string.Format(@"
                    SELECT 
                        ...
                        , '◈ ' + CASE WHEN A.eccb_tp = 'SCCB' THEN 'SCCB ' ELSE 'ECCB ' END + '심의 회의록 ◈' AS eccb_title
                    FROM EC_ECCB A
                ...
                if (objDr.Read())
                {
                    strBody =
                        ...
                        + "<font face=굴림체>" + objDr["eccb_title"] + "</font>"
			            + "</td>"
			            + "</tr>"
