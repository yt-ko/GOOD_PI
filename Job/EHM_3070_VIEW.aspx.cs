...
public partial class JOB_EHM_3070_VIEW : System.Web.UI.Page
{
    ...
    public static string Update(cSaveData DATA)
    {
        ...
        try
        {
            ...
            //---------------------------------------------------------------------------
            if (DATA.getFirst().getQuery() == "EHM_3070_M_2"
                && DATA.getFirst().getFirst().getType() == typeQuery.INSERT)
            {
                ...
                try
                {
                    ...
                    objProcedure.objCmd.Parameters.AddWithValue("@KeyType", "VOC");
                            ...
                            try
                            {
                                objUpdate.objDr = (new cDBQuery(
                                                        ...
                                                        "SELECT dbo.FN_CREATEKEY('" + strID + "','" +
                                                            DATA.getValue(iAry, iRow, "voc_no") + "')"
    ...
    #region SendMail() : Update Process
    ...
    /// </summary>
    [WebMethod]
    public static string SendMail(cSaveData DATA)
    {
        #region check Argument.
        ...
        try
        {
            ...
            string strPath = HttpContext.Current.Server.MapPath("..") + "/Files/VOC/Report/";
            ...
            try
            {
                ...
                {
                    if (DATA.getFirst().getValue(iAry, "send_yn") != "1")
                        continue;
